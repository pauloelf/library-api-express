...
import { MarkAsReadService } from '../services/MarkAsReadService.js'

export class MarkAsReadController {
  private markAsReadService = new MarkAsReadService()
  ...
  async handle(req: Request<{ bookId: string }>, res: Response) {
    ...
    const readBooks = await this.markAsReadService.execute(accountId, bookId)
    return created(res, readBooks)
  }
}
